...
final class SharedAccessSignatureHelper
{
    ...
    protected static UriQueryBuilder generateSharedAccessSignature(SharedAccessPolicy sharedaccesspolicy, 
        ...
        throws IllegalArgumentException, StorageException
    {
        ...
        {
            String signedpermissions = SharedAccessPolicy.permissionsToString(sharedaccesspolicy.permissions);
            if(Utility.isNullOrEmpty(signedpermissions))
                signedpermissions = null;
            String signedstart = Utility.getUTCTimeOrEmpty(sharedaccesspolicy.sharedAccessStartTime);
            if(!Utility.isNullOrEmpty(signedstart))
                uriquerybuilder.add("st", signedstart);
            String signedexpiry = Utility.getUTCTimeOrEmpty(sharedaccesspolicy.sharedAccessExpiryTime);
    ...
    protected static StorageCredentialsSharedAccessSignature parseQuery(HashMap<String, String[]> hashmap)
        throws IllegalArgumentException, StorageException
    {
        ...
        if (flag)
        {
            ...
            if(!Utility.isNullOrEmpty(signature))
                uriquerybuilder.add("sig", signature);
            credentialsSAS = new StorageCredentialsSharedAccessSignature(uriquerybuilder.toString());
        }
